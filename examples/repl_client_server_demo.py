...
Setup:
    ...
    Or using CLI:
    Terminal 1: $ py2max-server serve my-patch.maxpat
    Terminal 2: $ py2max-server repl localhost:8002 --token <token>
...
async def run_server():
    ...
    # Start REPL server
    from py2max_server.rpc import start_repl_server
    ...
    print(
        f"  PY2MAX_REPL_TOKEN={token} python examples/repl_client_server_demo.py client"
    )
    print("  OR")
    print(f"  py2max-server repl localhost:8002 --token {token}")
    print()
...
async def run_client():
    ...
    from py2max_server.client import start_repl_client

    # The session token is printed by the server; pass it via PY2MAX_REPL_TOKEN.
    token = os.environ.get("PY2MAX_REPL_TOKEN")
    if not token:
        print(
            "Set PY2MAX_REPL_TOKEN to the token printed by the server.",
            file=sys.stderr,
        )
        return

    # Connect to server
    await start_repl_client("localhost", 8002, token=token)
...
def print_usage():
    ...
    print("Or using CLI:")
    print()
    print("  Terminal 1: py2max-server serve my-patch.maxpat")
    print("  Terminal 2: py2max-server repl localhost:8002 --token <token>")
    print()
    print("=" * 70)
    print()
